...
WeatherService.prototype.getWeather = function(city, message) {
  if (city.length > 1) {
    ...
    this.getWeatherForCity('Ha noi', message);
  }
}

WeatherService.prototype.getWeatherForCity = function(city, message) {
  var vm = this;
  request('http://api.openweathermap.org/data/2.5/weather?q=' + xoa_dau(city) + '&APPID=' + vm.apikey + '&units=metric', (error, response, body) => {
...
function buildWeather(city, weather) {
  var toReturn = '\nThời tiết ở ' + capitalizeFirstLetter(city) + 'là: ';
